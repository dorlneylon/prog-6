 ...
 * It is innherited by the {@code MovieCol} class.
 *
 * @author dorlneylon
...
public abstract class MHMap<K, V> {
	...
	public boolean insert(K key, V value) {
		if (!this.map.containsKey(key)) { this.map.put(key, value); return true; }
		else { System.out.println("The value with this key already exists. Please, use the update command."); return false; }
	}
    ...
    public boolean rmByKey(K key) {
        if (containsKey(key)) { this.map.remove(key); return true; }
		else return false;
    }
    ...
    public void update(K key, V value) {
        if (containsKey(key)) { this.map.put(key, value); System.out.println("Done!"); }
